...
class Stream(override val client: PenicillinClient): Endpoint {
    ...
    fun siteStream(delimited: String? = null, stallWarnings: Boolean? = null, with: UserStreamWith? = null, replies: UserStreamReplies? = null, follow: List<Long>? = null, vararg options: Pair<String, Any?>) = client.session.getStream<UserStreamListener>("https://sitestream.twitter.com/1.1/site.json") {
    ...
    fun sampleStream(delimited: String? = null, stallWarnings: Boolean? = null, vararg options: Pair<String, Any?>) = client.session.getStream<SampleStreamListener>("https://stream.twitter.com/1.1/statuses/sample.json") {
        query("delimited" to delimited, "stall_warning" to stallWarnings, *options)
    }

    fun filterStream(delimited: String? = null, stallWarnings: Boolean? = null, track: List<String>? = null, follow: List<Long>? = null, locations: Pair<Float, Float>? = null, vararg options: Pair<String, Any?>) = client.session.postStream<FilterStreamListener>("https://stream.twitter.com/1.1/statuses/filter.json") {
