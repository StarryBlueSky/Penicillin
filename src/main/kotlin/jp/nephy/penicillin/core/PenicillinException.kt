...
enum class TwitterErrorMessage(val code: Int, val title: String, val description: String) {
    ...
    ThisApplicationIsNotAllowedToAccessOrDeleteYourDirectMessages(
    ...
    CannotFindSpecifiedUser(108, "Cannot find specified user", "Cannot find specified user."), UserNotInThisList(109, "User not in this list", "The specified user is not a member of this list."),
    AccountUpdateFailed(
        120,
    ...
    YouHaveAlreadyRequestedToFollowUser(
    ...
    ThisRequestLooksLikeItMightBeAutomated(
        226,
        "This request looks like it might be automated. To protect our users from spam and other malicious activity, we cannot complete this action right now.",
    ...
    UserMustVerifyLogin(
        ...
        "Returned as a challenge in xAuth when the user has login verification enabled on their account and needs to be directed to twitter.com to generate a temporary password. Note that xAuth is no longer an available option for authentication on the API."
    ),
    ...
    ApplicationCannotPerformWriteActions(
        261,
        "Application cannot perform write actions.",
        "Corresponds with HTTP 403. Thrown when the application is restricted from POST, PUT, or DELETE actions. Check the information on your application dashboard. You may also file a ticket atÂ https://help.twitter.com/forms/platform."
    ),
    YouCantMuteYourself(271, "You can't mute yourself.", "Corresponds with HTTP 403. The authenticated user account cannot mute itself."), YouAreNotMutingTheSpecifiedUser(
    ...
    AMediaIdWasNotFound(325, "A media id was not found.", "Corresponds with HTTP 400. The media ID attached to the Tweet was not found."), ThisAccountIsTemporarilyLocked(
        326,
