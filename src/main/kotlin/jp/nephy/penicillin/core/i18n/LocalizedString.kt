...
enum class LocalizedString(private val ja: String, private val en: String) {
    CredentialsAreAllNull(
        "ConsumerKey, ConsumerSecret, AccessToken, AccessTokenSecret, BearerTokenのすべてがnullです。", "ConsumerKey, ConsumerSecret, AccessToken, AccessTokenSecret and BearerToken are all null."
    ),
    PrivateEndpointRequiresOfficialClientEmulation(
        "非公開APIへのアクセスには公式クライアントのエミュレーションが必要です。", "Access to private apis requires official client's emulation."
    ),
    CursorIsZero(
        "カーソル(cursor)値は0です。", "cursor value is 0."
    ),
    ApiRequestFailedLog(
        "APIのリクエストに失敗しました。 (%s, %d/%d)", "Failed to request API. (%s, %d/%d)"
    ),
    ApiRequestFailed(
        "APIのリクエストに失敗しました: %s", "Failed to request API: %s"
    ),
    ApiReturnedNon200StatusCode(
        "APIが2xxではないステータスコードを返却しました: %s %s", "API returned non-200 status code: %s %s"
    ),
    JsonParsingFailed(
        "JSONをパースする際に例外が発生しました: %s", "Failed to parse JSON: %s"
    ),
    JsonModelCastFailed(
