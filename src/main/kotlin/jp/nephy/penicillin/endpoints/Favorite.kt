...
class Favorite(override val client: PenicillinClient): Endpoint {
    fun list(userId: Long? = null, screenName: String? = null, sinceId: Long? = null, maxId: Long? = null, count: Int? = null, includeEntities: Boolean? = null, vararg options: Pair<String, Any?>) = client.session.get("/1.1/favorites/list.json") {
        parameter("user_id" to userId, "screen_name" to screenName, "count" to count, "since_id" to sinceId, "max_id" to maxId, "include_entities" to includeEntities, *options)
    }.jsonArray<Status>()
